        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="font-heading text-[18px] sm:text-[20px] text-white">
                PASO 2: GENERAR PARTIDOS
              </h3>
              <span className="font-body text-[14px] text-blue-400">
                ({categoryName})
              </span>
            </div>
            <p className="font-body text-[13px] sm:text-[14px] text-gray-400">
              Las series de <span className="text-white font-semibold">{categoryName}</span> están listas. Ahora podés generar los partidos round-robin o crearlos manualmente.
            </p>
          </div>
          <button
            onClick={() => setShowConfirm(true)}
            disabled={generating}
            className="bg-[#dbf228] text-[#1b1b1b] font-heading text-[14px] sm:text-[16px] py-2 sm:py-3 px-4 sm:px-6 rounded hover:bg-[#c5db23] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 whitespace-nowrap min-w-fit"
          >
            {generating ? (
              <>
                <ButtonBallSpinner />
