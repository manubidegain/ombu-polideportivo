...
export function EditMatchModal({ match, onClose, onSuccess }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1b1b1b] border border-white/20 rounded-lg max-w-2xl w-full">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div className="flex items-center gap-2">
            <Calendar className="w-6 h-6 text-[#dbf228]" />
            <h2 className="font-heading text-[20px] text-white">EDITAR PARTIDO</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="bg-white/5 rounded-lg p-4 border border-white/10">
            <p className="font-body text-[14px] text-gray-400 mb-2">Partido:</p>
            <p className="font-heading text-[16px] text-white">
